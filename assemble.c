...
int symbolCount = 0;
int PC = 0;  // Global PC for tracking current address
...
int jmpjsrrInstruction(int opcode, char* arg1, char* arg2, char* arg3){
    ...
    // FIX 2: JMP needs bits 8:6 set to 000, JSRR needs bit 11 = 0
    if(opcode == 0xC) {
        // JMP: bits 11-9 = 000, bits 8-6 = BaseR, bits 5-0 = 000000
        num += getRegister(arg1) << 6;
    } else {
        // JSRR: bit 11 = 0, bits 8-6 = BaseR
        num += getRegister(arg1) << 6;
    }
...
int jsrInstruction(int opcode, char* arg1, int currAddr){
    ...
    // Handle both labels and immediate values
    if(arg1[0] == '#' || arg1[0] == 'x') {
        num += toNum(arg1) & 0x7FF;
    } else {
        // It's a label - calculate PC-relative offset
        int symAddr = findSymbol(arg1);
        if(symAddr == -1) {
            printf("Error: undefined symbol '%s'\n", arg1);
            exit(4);
        }
        // FIX 3: Correct PC-relative offset calculation
...
int ldbldwstbstwInstruction(int opcode, char* arg1, char* arg2, char* arg3){
    ...
    // For STB/STW, source register goes in bits 11-9
    // For LDB/LDW, destination register goes in bits 11-9
    ...
    num += getRegister(arg2) << 6;
        ...
        // FIX 4: Handle negative immediate values properly
...
int leaInstruction(int opcode, char* arg1, char* arg2, int currAddr){
    ...
    if(arg2[0] == '#' || arg2[0] == 'x') {
        ...
        // FIX 5: Correct PC-relative offset calculation
        int offset = (symAddr - (currAddr + 2)) / 2;
...
int notInstruction(int opcode, char* arg1, char* arg2){
    ...
    num += getRegister(arg1) << 9;
    num += getRegister(arg2) << 6;
    num += 0x3F;  // bits 5-0 all set to 1
    return num;
}
...
int retrtiInstruction(int opcode, char* arg1, char* arg2){
    int num = 0;
    num += opcode << 12;
    if(opcode == 0xC) num += 0x7 << 6;  // RET uses R7
    return num;
}
...
int brInstruction(int opcode, char* arg1, char* conditionBits, int currAddr){
    ...
    if(arg1[0] == '#' || arg1[0] == 'x') {
        ...
        // FIX 7: Correct PC-relative offset calculation
        int offset = (symAddr - (currAddr + 2)) / 2;
